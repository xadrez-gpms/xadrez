...
Torre E -> Torre do lado esquerdo do Rei (mais distante)
Torre D -> Torre do lado direito do Rei (mais próxima)
...
def movimentaPeca(tab, type, x_ori, y_ori, x_dest, y_dest):
    if checaMovimentaPeça(tab, type, x_ori, y_ori, x_dest, y_dest):
        ...
        if(type == BR or type == PR or type == BT or type == PT): # Se for movimentação do peão ou do rei, ajusta o dicionário para o roque
